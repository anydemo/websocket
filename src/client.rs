...
use crate::interface::WebSocketEvent;
...
impl WebSocketClient {
    ...
    fn write_handshake(&mut self) {
        ...
        self.tx
            .lock()
            .unwrap()
            .send(WebSocketEvent::Connect(self.token));
                ...
                match frame.get_opcode() {
                    OpCode::TextFrame => {
                        ...
                        self.tx
                            .lock()
                            .unwrap()
                            .send(WebSocketEvent::TextMessage(self.token, payload));
                    }
                    OpCode::BinaryFrame => {
                        self.tx
                            .lock()
                            .unwrap()
                            .send(WebSocketEvent::BinaryMessage(self.token, frame.payload));
                    }
                    OpCode::Ping => {
                    ...
                    OpCode::ConnectionClose => {
                        self.tx
                            .lock()
                            .unwrap()
                            .send(WebSocketEvent::Close(self.token));
