...
impl WebSocketFrame {
    pub fn close(status_code: u16, reason: &[u8]) -> Result<WebSocketFrame, String> {
        ...
        body_cursor
            .write_u16::<BigEndian>(status_code)
        ...
        Ok(WebSocketFrame {
            header: WebSocketFrameHeader::new_header(
                body_cursor.get_ref().len(),
    ...
    pub fn read<R: Read>(input: &mut R) -> io::Result<WebSocketFrame> {
        ...
        let header = Self::parse_header(buf).map_err(|s| io::Error::new(ErrorKind::Other, s))?;
