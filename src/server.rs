...
unsafe impl Sync for WebSocketServerState {}
unsafe impl Send for WebSocketServerState {}
...
impl WebSocketServer {
    pub fn new(
        socket: TcpListener,
        shared_state: WebSocketServerState,
        tx: mpsc::Sender<WebSocketEvent>,
    ) -> WebSocketServer {
        WebSocketServer {
            socket: socket,
        ...
        if events.is_readable() {
            match token {
                SERVER_TOKEN => {
                    ...
                    let new_token =
                        self.state
                            .new_client(client_socket, self.tx.clone(), event_loop.channel());
